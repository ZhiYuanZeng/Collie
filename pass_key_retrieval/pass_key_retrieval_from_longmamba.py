...
def parse_config():
    ...
    parser.add_argument('--chunk_size', type=int, default=1024, help='number of repeat testing for each length')
    parser.add_argument('--memory_size', type=int, default=1024, help='number of repeat testing for each length')
    parser.add_argument('--pruner_type', type=str, default="streaming_llm", help='number of repeat testing for each length')
    parser.add_argument('--memory_type', type=str, default="streaming_llm", help='number of repeat testing for each length')
...
def build_model(model_path, chunk_size, memory_size_limit, memory_type, pruner_type):
    ...
    model = AutoMemory.from_pretrained(
        ...
        config=config).cuda().to(torch.bfloat16)
    return model
...
def main(args):
    ...
    model = build_model(model_path=args.base_model, chunk_size=args.chunk_size, memory_size_limit=args.memory_size, memory_type=args.memory_type, pruner_type=args.pruner_type)
    ...
    # save
    plt.savefig(f"imgs/heatmap_{args.max_tokens}.png".format(args.max_tokens))
