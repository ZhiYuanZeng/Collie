...
def eval_speed(pruner_type, memory_type, chunk_size, compressed_chunk_size, memory_size_limit, input_len, generate_len):
    ...
    print(f'chunk_size: {chunk_size} | compressed_chunk_size: {ccs} | memory size limit: {ms} | \
...
# chunk_sizes = [512]
# memory_size_limits=[256, 512, 1024, 2048]
# input_lens = [16384]
# generate_lens = [1]
...
pruner_types = [PrunerType.TOVA, PrunerType.CONV]
memory_types = [MemoryType.CHUNK_STREAMING]
chunk_sizes = [512]
memory_size_limits=[512]
compressed_chunk_sizes=[512]
input_lens = [16384]
generate_lens = [1]
for ms in memory_size_limits:
    for chunk_size in chunk_sizes:
        for ccs in compressed_chunk_sizes:
            for memory_type in memory_types:
                for pruner_type in pruner_types:
                    for input_len in input_lens:
                        for generate_len in generate_lens:
